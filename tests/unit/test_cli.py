...
class TestCli(UnitTestCase):
    ...
    def test_missing_credentials(self):
        ...
        response = self.runner.invoke(
            amazon_orders_cli, ["--config-path", self.test_config.config_path, "--username", "", "--password", ""]
        )
    ...
    def test_logout_command(self):
        ...
        # WHEN
        response = self.runner.invoke(amazon_orders_cli, ["--config-path", self.test_config.config_path, "logout"])
    ...
    def test_check_session_command(self):
        ...
        # WHEN
        response = self.runner.invoke(
            amazon_orders_cli, ["--config-path", self.test_config.config_path, "check-session"]
        )
    ...
    def test_order_command(self):
        ...
        self.given_login_responses_success()
        with open(
            os.path.join(self.RESOURCES_DIR, "orders", "order-details-112-2961628-4757846.html"), encoding="utf-8"
        ) as f:
            resp1 = responses.add(
                responses.GET,
    ...
    def test_persisted_session_stale_logout(self):
        ...
        # WHEN
        response = self.runner.invoke(amazon_orders_cli, ["--config-path", self.test_config.config_path, "history"])
    ...
    def test_update_config(self):
        # GIVEN
        self.test_config.save()
        with open(self.test_config.config_path) as f:
            self.assertIn("max_auth_attempts: 10", f.read())

        # WHEN
        response = self.runner.invoke(
            amazon_orders_cli,
            ["--config-path", self.test_config.config_path, "update-config", "max_auth_attempts", "7"],
        )
