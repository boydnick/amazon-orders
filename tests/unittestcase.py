...
class UnitTestCase(TestCase):
    RESOURCES_DIR = os.path.normpath(os.path.join(os.path.abspath(os.path.dirname(__file__)), "resources"))
    ...
    def setUp(self):
        ...
        self.test_config = AmazonOrdersConfig(
            data={
    ...
    def given_authenticated_url_redirects_to_login(self, method=responses.GET):
    ...
    def given_authenticated_url_renders_login(self, method=responses.GET):
        with open(os.path.join(self.RESOURCES_DIR, "auth", "signin.html"), encoding="utf-8") as f:
            return responses.add(
                method,
                re.compile(f"{self.test_config.constants.BASE_URL}/(gp|cpe|your-orders)/.*"),
    ...
    def given_order_history_exists(self, year, start_index=0):
        with open(
            os.path.join(self.RESOURCES_DIR, "orders", f"order-history-{year}-{start_index}.html"), encoding="utf-8"
        ) as f:
            optional_start_index = f"&startIndex={start_index}" if start_index else ""
            return responses.add(
                responses.GET,
                f"{self.test_config.constants.ORDER_HISTORY_URL}?timeFilter=year-{year}{optional_start_index}",
                body=f.read(),
                status=200,
            )

    def given_any_order_history_exists(self, order_history_html_file):
        with open(os.path.join(self.RESOURCES_DIR, "orders", order_history_html_file), encoding="utf-8") as f:
            return responses.add(
                responses.GET,
    ...
    def given_any_order_details_exists(self, order_details_html_file):
        with open(os.path.join(self.RESOURCES_DIR, "orders", order_details_html_file), encoding="utf-8") as f:
            return responses.add(
                responses.GET,
